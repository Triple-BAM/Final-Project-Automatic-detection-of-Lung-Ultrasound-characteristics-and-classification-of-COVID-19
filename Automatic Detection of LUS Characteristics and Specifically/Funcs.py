...
def PL_Detect(path, W, plot=False):
    """
    ...
    :param plot: If plot=True, plots images that help with debugging
    ...
    for i in np.arange(len(Vid)):
        ...
        image = crop_LUS(image=image, plot=plot)
        ...
        Boundary_Box_PL_Mask = Boundary_Box_PL(cropped_image=(Seg_PL_Mask * im) > 0.9, line_thickness=int(W / 20), plot=plot)

        if plot:
            plt.imshow(im)
            plt.imshow(Boundary_Box_PL_Mask * im, alpha=0.5)
            plt.title('Boundary Box on original image')
            plt.show()
...
def crop_LUS(image, plot=False):
    ...
    if plot:
        ...
        ax[3].set_title('Image after vertical/horizontal cuts')
        plt.show()
...
def Boundary_Box_PL(cropped_image, line_thickness, plot=False):
    """
    ...
    if plot:
        ...
        ax[0].set_title('Thresholded cropped image')
    ...
    if plot:
        ax[1].imshow(BB_Mask)
        ax[1].imshow(cropped_image, alpha=0.5)
        ax[1].set_title('Boundary Box on thresholded cropped image')
        plt.show()
    return BB_Mask
